...
The production target is a single-origin FastAPI service serving the Vite `dist/` at `/` and API routes under `/api`. `Dockerfile` builds the frontend and packages the validated backend, model, processed data, scenarios, and OSM demo graph. Liveness is `/healthz`; the Render/container health check must be `/readyz`; product detail is `/api/release/readiness`.
