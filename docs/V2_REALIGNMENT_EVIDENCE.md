...
- `npx vitest run --pool=threads --maxWorkers=1 --reporter=dot`: 10 files, 43 tests passed.
...
- Playwright `v2-map.spec.ts` 및 `v2-event-public.spec.ts`: 이전 desktop targeted run에서 3D source/layer 1개와 행사 3개가 통과했다. 공개 행사 페이지, QR target 실제 재접속, 장면 전환, EBML WebM download, 실내 image-local 경로 그리기, 야외 MapLibre 경로·AED 공개 전환을 포함한다. 추가 headless production browser smoke에서 실내 SVG 업로드·자동 초안과 야외 OSM 후보 2개·초안 적용을 실제 화면으로 확인했다.
...
- production recovery 재검증: 최신 변경 후 same-origin FastAPI runtime에서 `production-recovery.spec.ts` 2 tests passed (28.5s). `/healthz`, `/readyz`, `/api/release/version`, AED endpoint, 시민 도보 geometry, 이벤트 route, 정적 훈련 화면, 관리자 exact/AI 경로를 확인했다.
...
- `artifacts/evals/goal7a/public-training-semantic-t0.png`
- `artifacts/evals/goal7a/public-training-semantic-t30.png`
