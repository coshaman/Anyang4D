...
export function AdminSimulator({ onBack, demoMode = false }: { onBack: () => void; demoMode?: boolean }) {
  ...
  async function loadScenarios() {
    try {
      ...
      setStatus("API 연결 전에도 데모 화면을 확인할 수 있습니다. 서버를 연결하면 frame을 계산합니다.");
    }
  }
  ...
  useEffect(() => { void fetch(`${API}/resources`).then((response) => response.json()).then((body) => { setResourceCount(body.count); setResources(body.items); }).catch(() => setStatus("공식 자원 맥락을 불러오지 못했습니다.")); }, []);
