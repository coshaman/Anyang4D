...
export function AdminSimulator({ onBack, demoMode = false }: { onBack: () => void; demoMode?: boolean }) {
  ...
  return <main className="admin-page" aria-label="안양 행정 4D 시나리오 시뮬레이터">
    ...
    <aside className="admin-pane">
      ...
      {workspace === "advanced" && demoMode && <section className="admin-editor" aria-label="AI 대규모 시나리오 선별"><h2>AI 대규모 시나리오 선별</h2><p className="scenario-assumption">후보 생성 → AI 빠른 선별 → 상위 후보 exact 재검증. 모두 SIMULATED_ADMIN_SCENARIO이며 시민 안내가 아닙니다.</p><div className="admin-controls"><label>후보 수<select aria-label="AI 후보 시나리오 수" value={aiCandidateCount} onChange={(event) => setAiCandidateCount(Number(event.target.value))}><option value="100">100</option><option value="500">500</option><option value="1000">1,000</option></select></label><button className="secondary-button" type="button" onClick={() => void screenAi()}>AI 빠른 선별</button></div>{aiStatus && <p className="admin-status" role="status">{aiStatus}</p>}{aiScreen && <><p className="scenario-assumption">{aiScreen.candidate_count.toLocaleString()}개 후보 · exact 호출 {aiScreen.exact_calls}회 · 모델 {aiScreen.model_version}</p><ol className="ai-shortlist">{aiScreen.verified_shortlist.map((item: Scenario, index: number) => <li key={item.scenario_id}><strong>#{index + 1} {item.scenario_id}</strong><span>AI 추정 미배정 {Math.round(item.estimated_unserved ?? 0).toLocaleString()}명 · exact 미배정 {item.exact_result?.unserved?.toLocaleString?.() ?? "-"}명 · {item.support_status}</span><small>주요 입력: {(item.explanation || []).slice(0, 2).map((reason: Scenario) => reason.feature).join(", ") || "없음"}</small></li>)}</ol><p className="scenario-assumption">표시 최종값은 exact reference 결과이며 AI 추정치는 별도 감사용으로 보존합니다.</p></>}</section>}
