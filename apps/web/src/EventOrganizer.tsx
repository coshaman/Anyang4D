...
import { addEventNode, addRoutePoint, createEventPlan, type EventPlan, type EventPoint, type EventRepresentation } from "./eventPlan";
...
export function EventOrganizer({ onPreview, onBack }: { onPreview: (plan: EventPlan) => void; onBack?: () => void }) {
  ...
  function preview() { onPreview({ ...plan, name, venue, representation, emergencyInstructions: selectedInstructions, slug: name.trim().toLowerCase().replace(/\s+/g, "-") || "event" }); }
