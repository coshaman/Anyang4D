...
export function buildOutdoorRouteGeometry(start: OutdoorPoint | null | undefined, exit: OutdoorPoint | null | undefined, route: OutdoorPoint[] = [], assembly?: OutdoorPoint | null) {
  return [start, ...route, exit, assembly].filter((point): point is OutdoorPoint => Boolean(point));
}
...
export function EventOrganizer({ onPreview, onBack }: { onPreview: (plan: EventPlan) => void; onBack?: () => void }) {
  ...
  async function makeOutdoorDraft() {
    if (!activeGroup.outdoorStart || !activeGroup.outdoorExit) {
      setDraftStatus("먼저 ① 참가자 시작 구역과 ② 비상 출구를 각각 지정하세요. 현재 빠진 항목을 지도에서 선택합니다.");
      setNodeKind(activeGroup.outdoorStart ? "exit" : "start");
      setDrawingRoute(false);
      return;
    }
    setDraftStatus("OSM 도보 그래프에서 자동 초안을 계산하는 중입니다…");
    try {
      const request = (origin: OutdoorPoint, destination: OutdoorPoint) => fetchJson<RouteCandidate>(`${API_BASE}/routes`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ origin, destination }) }, 45000, 0);
  ...
  const outdoorGeometry = buildOutdoorRouteGeometry(activeGroup.outdoorStart, activeGroup.outdoorExit, outdoorRoute, activeGroup.outdoorAssembly);
  ...
  return <main className="event-organizer" aria-label="행사 대피안내 만들기">
    ...
    {step >= 3 && <section className="event-step"><h2>3. 구역과 안내 경로</h2><div className="admin-controls"><label>구역<select aria-label="행사 구역" value={groupId} onChange={(event) => setGroupId(event.target.value)}>{plan.groups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label><label>표시<select aria-label="안내 지점 종류" value={nodeKind} onChange={(event) => setNodeKind(event.target.value as NodeKind)}><option value="start">참가자 시작 구역</option><option value="exit">비상 출구</option><option value="assembly">집결지</option><option value="AED">AED</option><option value="EXTINGUISHER">소화기</option><option value="STAIRS">계단</option><option value="RESTRICTED_ZONE">출입 제한 구역</option></select></label><button className="secondary-button" type="button" onClick={() => setDrawingRoute((value) => !value)}>{drawingRoute ? "경로 그리기 종료" : "경로 그리기"}</button><button className="secondary-button" type="button" disabled={!activeRouteLength} onClick={() => updatePlan(representation === "OUTDOOR" ? removeLastOutdoorRoutePoint(plan, groupId) : removeLastRoutePoint(plan, groupId))}>마지막 경로점 삭제</button></div>
      ...
      {representation === "OUTDOOR" && <><section className="event-route-checklist" aria-label="야외 경로 작성 단계"><strong>야외 경로 작성</strong><div><button type="button" className={activeGroup.outdoorStart ? "route-step done" : "route-step"} onClick={() => { setNodeKind("start"); setDrawingRoute(false); }}>{activeGroup.outdoorStart ? "✓ 시작 구역 지정됨" : "1 시작 구역 지정"}</button><button type="button" className={activeGroup.outdoorExit ? "route-step done" : "route-step"} onClick={() => { setNodeKind("exit"); setDrawingRoute(false); }}>{activeGroup.outdoorExit ? "✓ 출구 지정됨" : "2 비상 출구 지정"}</button><button type="button" className={outdoorRoute.length > 1 ? "route-step done" : "route-step"} onClick={() => setDrawingRoute(true)}>{outdoorRoute.length > 1 ? `✓ 수동 경로 ${outdoorRoute.length}점` : "3 수동 경로 그리기"}</button></div><p>{activeGroup.outdoorStart && activeGroup.outdoorExit ? "시작 구역과 출구가 준비되었습니다. 자동 초안을 만들거나 수동 경로를 그리세요." : "지도에서 위 단계를 순서대로 선택하면 클릭 모드가 자동으로 맞춰집니다."}</p></section><div className="event-outdoor-editor"><MapView facilities={[]} onSelect={() => undefined} currentLocation={null} eventSafetyPoints={plan.outdoorFacilities} walkingRoute={outdoorGeometry.length > 1 ? { geometry: outdoorGeometry, origin: outdoorGeometry[0], destination: outdoorGeometry[outdoorGeometry.length - 1] } : null} onMapClick={placeOutdoorPoint} /><p className="event-map-hint">지도 클릭은 현재 선택한 단계에 입력됩니다. 시작/출구를 먼저 지정하고, 수동 경로는 경로 그리기를 켠 뒤 두 점 이상 클릭하세요.</p></div><button className="primary-button" type="button" onClick={() => void makeOutdoorDraft()}>자동 초안 경로 만들기</button><p className="field-hint">OSM 도보 그래프 기반 참고 초안입니다. 운영자가 실제 현장 조건을 검토·수정하세요.</p>{outdoorDrafts.map((draft, index) => <button className="secondary-button" type="button" key={`${draft.distance_m}-${index}`} onClick={() => applyOutdoorDraft(index)}>초안 적용 {index + 1} · 약 {draft.distance_m.toLocaleString()}m · {draft.estimated_walking_minutes}분</button>)}</>}
