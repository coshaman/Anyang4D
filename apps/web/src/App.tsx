...
export function App() {
  ...
  const nearby = visibleFacilities.filter((facility) => facility.latitude !== null).slice(0, 5);
  ...
  function selectCategory(next: Category) { setCategory(next); setSelected(null); setRouteMessage(""); }
  async function requestRoute() { if (!selected?.latitude || !selected.longitude) return; const origin = location || center; try { const response = await fetch(`${API_BASE}/routes`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ origin, destination: { latitude: selected.latitude, longitude: selected.longitude } }) }); if (!response.ok) throw new Error(); const route = await response.json(); setRouteMessage(`기본 도보 경로 · 약 ${route.distance_m.toLocaleString()}m · ${route.estimated_walking_minutes}분`); } catch { setRouteMessage("도보 경로를 불러오지 못했습니다. 현재 연결된 보행 네트워크를 확인하세요."); } }
