...
export function CitizenSimulationPreview({ onBack }: { onBack: () => void }) {
  ...
  const availabilityById = useMemo(() => {
    const simulated = frame?.facilities[0];
    const firstShelter = shelterFacilities[0];
    return firstShelter && simulated ? { [firstShelter.id]: simulated.available } : {};
  }, [frame, shelterFacilities]);
  ...
  return <main className="simple-page citizen-training-page">
    ...
    {scenario && <>
      ...
      <MapView facilities={shelterFacilities} onSelect={() => undefined} currentLocation={null} hazardGeometry={frame?.hazard.geometry} hazardLabel={frame?.hazard.label} changedRoads={frame?.roads.changed} facilityLoads={loadById} facilityLoadRatios={loadById} facilityAvailability={availabilityById} evacuationFlow={frame?.evacuation_flow} />
